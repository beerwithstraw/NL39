"""
pipeline.py — Config-driven NL-39 extraction pipeline.
...
from extractor.consolidated_detector import find_nl39_pages, extract_nl39_to_temp
...
def main():
    parser = argparse.ArgumentParser(description="NL-39 Config-Driven Extraction Pipeline")
    ...
    for result in to_process:
        ...
        try:
            ...
            if result.source_type == "consolidated" and consolidated_mode != "skip":
                page_overrides = config.get("nl39_page_overrides", {})
                override = page_overrides.get(result.company_key) or {}
                keywords = config.get("nl39_keywords", None)
                ...
                if "start" in override:
                    ...
                    min_matches = int(override.get(
                        "min_matches",
                        config.get("nl39_keyword_min_matches", 2),
                    ))
                    pages = find_nl39_pages(result.pdf_path, keywords, min_matches)

                if pages is None:
                    logger.warning(
                        f"  Could not find NL-39 section in consolidated PDF, skipping: "
                        f"{result.pdf_path}"
                    )
                    failed += 1
                    failed_files.append((result.pdf_path, "NL-39 section not found"))
                    continue

                temp_file = extract_nl39_to_temp(result.pdf_path, pages[0], pages[1])
    ...
    if all_extractions:
        ...
        report_path = os.path.join(
            os.path.dirname(master_path), "NL39_validation_report.csv"
        )
