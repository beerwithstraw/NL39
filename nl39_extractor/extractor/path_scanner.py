...
def scan(config: Dict[str, Any]) -> List[ScanResult]:
    ...
    Priority rule: if a company has a direct NL4/ file, it takes priority
    ...
    for fy in fiscal_years:
        ...
        for quarter in quarters:
            ...
            nl39_path = os.path.join(q_path, "NL39")
            consolidated_path = os.path.join(q_path, "Consolidated")
            direct_companies = set()

            # --- Scan NL39/ subfolder: any .pdf here is a direct NL-39 form ---
            ...
            # Only pick up consolidated PDFs for companies that don't have a direct NL39 file
            if os.path.isdir(consolidated_path):
                for fname in os.listdir(consolidated_path):
                    ...
                    if company_key in direct_companies:
                        logger.debug(
                            f"Skipping consolidated {fname} — direct NL39 exists for {company_key}"
                        )
                        continue
