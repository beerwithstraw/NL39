"""
Global constants and configuration for the NL-39 Extractor.
"""

# --- Versioning ---
EXTRACTOR_VERSION = "1.0.0"

# --- Default Paths ---
DEFAULT_INPUT_DIR = "inputs"
DEFAULT_OUTPUT_DIR = "outputs"

# --- FY Year String Helper ---

def make_fy_string(start_year: int, end_year: int) -> str:
    """Build the 6-character FY string. e.g. start=2025, end=2026 → '202526'"""
    return f"{start_year}{end_year % 100:02d}"

QUARTER_TO_FY = {
    "Q1": lambda y: make_fy_string(y, y + 1),
    "Q2": lambda y: make_fy_string(y, y + 1),
    "Q3": lambda y: make_fy_string(y, y + 1),
    "Q4": lambda y: make_fy_string(y - 1, y),
}

# --- Master Sheet Column Order ---
# NL-39 has no CY/PY split — single period. No Year_Info column.
# Quarter_Info = "For the Quarter" or "Up to the Quarter"
# Metric column names must .lower() to canonical metric keys in row_registry.py
# e.g. "Count_Upto_1M".lower() == "count_upto_1m" ✓
MASTER_COLUMNS = [
    "LOB_PARTICULARS",          # A
    "Grouped_LOB",              # B
    "Company_Name",             # C
    "Company",                  # D
    "NL",                       # E — always "NL39"
    "Quarter",                  # F
    "Year",                     # G — FY end year
    "Quarter_Info",             # H — "For the Quarter" / "Up to the Quarter"
    "Sector",                   # I
    "Industry_Competitors",     # J
    "GI_Companies",             # K
    # --- No. of claims paid ---
    "Count_Upto_1M",            # L
    "Count_1M_3M",              # M
    "Count_3M_6M",              # N
    "Count_6M_1Y",              # O
    "Count_1Y_3Y",              # P
    "Count_3Y_5Y",              # Q
    "Count_Gt5Y",               # R
    # --- Amount of claims paid ---
    "Amount_Upto_1M",           # S
    "Amount_1M_3M",             # T
    "Amount_3M_6M",             # U
    "Amount_6M_1Y",             # V
    "Amount_1Y_3Y",             # W
    "Amount_3Y_5Y",             # X
    "Amount_Gt5Y",              # Y
    # --- Totals ---
    "Total_Count",              # Z
    "Total_Amount",             # AA
    "Source_File",              # AB
]

# --- Excel Formatting ---
NUMBER_FORMAT = "#,##0.00"
LOW_CONFIDENCE_FILL_COLOR = "FFFF99"

# --- Common Helpers ---

def company_key_to_pascal(company_key: str) -> str:
    """Convert snake_case company key to PascalCase. e.g. 'bajaj_allianz' → 'BajajAllianz'"""
    return company_key.replace("_", " ").title().replace(" ", "")
